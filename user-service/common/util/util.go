...
func BindFromConsul(dest any, endPoint, path string)error{
	...
	v.SetConfigType("json")
	err := v.AddRemoteProvider("consule", endPoint, path)
	if err !=nil{
		logrus.Errorf("failed to add remote provide: %v", err)
		return err
	}
	...
	err = v.Unmarshal(&dest)
	if err != nil{
		logrus.Errorf("failed to read unmarshal: %v", err)
		return err
	}

	err=SetEnvFromConsulKV(v)
	if err != nil{
		logrus.Errorf("failed to set env consul kv : %v", err)
		return err
	}
